...
export function calculateLineAmount(item: Omit<LineItem, 'amount' | 'id'>): number {
...
export function calculateTotals(items: LineItem[], config: TotalsConfig): TotalsResult {
  ...
  const tax1Amount = config.tax1.enabled ? afterDiscount * (config.tax1.rate / 100) : 0
  const tax2Amount = config.tax2.enabled ? afterDiscount * (config.tax2.rate / 100) : 0

  const total =
    afterDiscount + tax1Amount + tax2Amount + config.shipping + config.adjustment
