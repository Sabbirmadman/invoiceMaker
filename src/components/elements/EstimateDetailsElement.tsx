    ...
    return (
      <div className="flex flex-col gap-1" style={element.styles as React.CSSProperties}>
        ...
        <div className="flex flex-col gap-1 text-sm">
          <div><span className={LABEL_STYLE}>Estimate #</span>
            <InlineField value={meta.number} onChange={(v) => onUpdateEstimateMeta({ number: v })} placeholder="EST-001" className={VALUE_STYLE} />
          </div>
          <div><span className={LABEL_STYLE}>Date</span>
            <input type="date" value={meta.date} onChange={(e) => onUpdateEstimateMeta({ date: e.target.value })} className={DATE_INPUT} />
          </div>
          <div><span className={LABEL_STYLE}>Expiry Date</span>
            <input type="date" value={meta.expiryDate ?? ''} onChange={(e) => onUpdateEstimateMeta({ expiryDate: e.target.value })} className={DATE_INPUT} />
          </div>
          <div><span className={LABEL_STYLE}>Reference</span>
            <InlineField value={meta.reference ?? ''} onChange={(v) => onUpdateEstimateMeta({ reference: v })} placeholder="Ref #" className={VALUE_STYLE} />
          </div>
        </div>
      </div>
    )
