            ...
            >
                <div style={gridStyle}>
                    {f("name") && (
                        <InlineField
                            value={company.name}
                            onChange={(v) => onUpdateCompany({ name: v })}
                    ...
                    {f("address") && (
                        <InlineField
                            ...
                            onChange={(v) => onUpdateCompany({ address: v })}
                            placeholder="Address"
                        />
                    )}
                    {f("cityStateZip") && (
                        <div className="flex gap-1">
                            <InlineField
                                value={company.city}
                                onChange={(v) => onUpdateCompany({ city: v })}
                                placeholder="City"
                            />
                            <InlineField
                                value={company.state}
                                onChange={(v) => onUpdateCompany({ state: v })}
                                placeholder="State"
                                className="w-16"
                            />
                            <InlineField
                                value={company.zip}
                                onChange={(v) => onUpdateCompany({ zip: v })}
                                placeholder="ZIP"
                                className="w-20"
                            />
                        </div>
                    ...
                    {f("country") && (
                        <InlineField
                            ...
                            onChange={(v) => onUpdateCompany({ country: v })}
                            placeholder="Country"
                        />
                    )}
                    {f("phone") && (
                        <InlineField
                            value={company.phone}
                            onChange={(v) => onUpdateCompany({ phone: v })}
                            placeholder="Phone"
                        />
                    )}
                    {f("email") && (
                        <InlineField
                            value={company.email}
                            onChange={(v) => onUpdateCompany({ email: v })}
                            placeholder="Email"
                        />
                    )}
                    {f("website") && (
                        <InlineField
                            ...
                            onChange={(v) => onUpdateCompany({ website: v })}
                            placeholder="Website"
                        />
                    )}
                    {f("taxId") && (
                        <InlineField
                            value={company.taxId}
                            onChange={(v) => onUpdateCompany({ taxId: v })}
                            placeholder="Tax ID"
                        />
                    )}
                </div>
