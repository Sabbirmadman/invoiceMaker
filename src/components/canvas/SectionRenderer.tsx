...
function BoundedCell({
  ...
  return (
    <div ref={ref} className={className} style={style}>
      {children}
      {showBounds && (
        <ElementBoundingBox elementRef={ref} color={SECTION_COLORS[sectionType]} />
      )}
    </div>
  )
}
...
export function BodySectionRenderer({
  ...
  const postTableElements = sorted.filter((el) => el.type !== 'watermark' && !isPreTable(el))
  // Slice to only the elements assigned to this page
  const visiblePostElements = postTableEndIndex !== undefined
    ...
    <div className="relative flex flex-col p-4 overflow-hidden">
      ...
      <div className="flex flex-col gap-4">
        {sorted.map((el) => {
          ...
          if ((el.placement ?? 'last-page') === 'first-page' && !isFirstPage) return null
          if (el.placement === 'all-pages' && doc.data.items.length === 0 && !isFirstPage) return null
          return (
            <BoundedCell
              key={el.id}
              sectionType="body"
