...
export default function HomePage() {
    ...
    const totalAmount = documents.reduce((sum, doc) => {
        ...
        const tax1 = totalsConfig.tax1.enabled
            ? sub * (totalsConfig.tax1.rate / 100)
            : 0;
        const tax2 = totalsConfig.tax2.enabled
            ? sub * (totalsConfig.tax2.rate / 100)
            : 0;
        return (
            sum +
            sub +
            tax1 +
            tax2 +
            totalsConfig.shipping +
            totalsConfig.adjustment
        );
    }, 0);
